...
function Box({ x, y, w, h, title, sub, accent }: { x: number; y: number; w: number; h: number; title: string; sub?: string; accent?: string }) {
  return (
    <g>
      <rect x={x} y={y} width={w} height={h} rx="6" fill="var(--color-mantle)" stroke={accent ?? "var(--color-surface1)"} />
...
function Arrow({ d, label, lx, ly }: { d: string; label?: string; lx?: number; ly?: number }) {
  return (
    <g>
      <path d={d} fill="none" stroke="var(--color-overlay)" strokeWidth="1.5" markerEnd="url(#arrow)" />
      {label ? (
        <text x={lx} y={ly} textAnchor="middle" fill="var(--color-overlay)" fontSize="10" fontFamily="var(--font-mono)">
          {label}
        </text>
      ) : null}
    </g>
...
export default function Architecture() {
  return (
    <section id="architecture" className="scroll-mt-8 border-b border-surface0">
      <div className="mx-auto max-w-5xl px-6 py-20">
        <SectionHeader tag="architecture" title="How a change ships" />
        <div className="overflow-x-auto rounded-lg border border-surface0 bg-crust p-4">
          <svg viewBox="0 0 860 380" className="min-w-[760px] w-full" role="img" aria-labelledby="arch-title arch-desc">
            ...
            <defs>
              <marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">
                <path d="M0,0 L8,4 L0,8 z" fill="var(--color-overlay)" />
              </marker>
            </defs>

            {/* zones */}
            <rect x="16" y="16" width="380" height="150" rx="8" fill="none" stroke="var(--color-surface0)" strokeDasharray="4 4" />
            <text x="28" y="36" fill="var(--color-blue)" fontSize="11" fontFamily="var(--font-mono)">GitHub</text>
            <rect x="16" y="196" width="580" height="164" rx="8" fill="none" stroke="var(--color-surface0)" strokeDasharray="4 4" />
            <text x="28" y="216" fill="var(--color-green)" fontSize="11" fontFamily="var(--font-mono)">Homelab · K3s cluster</text>
            <rect x="620" y="196" width="224" height="164" rx="8" fill="none" stroke="var(--color-surface0)" strokeDasharray="4 4" />
            <text x="632" y="216" fill="var(--color-peach)" fontSize="11" fontFamily="var(--font-mono)">AWS · eu-central-1</text>
            ...
            <Box x={756} y={56} w={88} h={44} title="Cloudflare" sub="edge · dns" accent="var(--color-yellow)" />
        ...
        <ul className="mt-6 grid gap-3 sm:grid-cols-3">
          {FACTS.map((fact) => (
            <li key={fact} className="rounded-lg border border-surface0 bg-mantle p-4 text-sm text-sub">
              {fact}
            </li>
          ))}
        </ul>
