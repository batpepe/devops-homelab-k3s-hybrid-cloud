...
| --- | --- | --- | --- |
| 2026-07-22 | CodeQL triage: first real findings fixed | SSRF in the museum image proxy closed with `redirect: "manual"` (the allowlist check ran after fetch had already followed the redirect); `express-rate-limit` on the three DB-backed `/api` routes keyed on `CF-Connecting-IP`; wildcard CORS default replaced by an origin allowlist; smoke test added to `ci-nodejs.yml` first so the change had a gate; actionlint findings cleared and the job promoted to enforcing; stored-xss pair dismissed as false positive with the rationale in SECURITY.md | locally built image: /health 200, 429 from request 121, second client IP unaffected, allowed origin echoed and unknown origin refused; `make lint-actions` exit 0 |
| 2026-07-11 | CI/DX hardening sprint | actionlint + yamllint jobs (report-only) and an enforcing trivy config gate in lint.yml; both trivy findings triaged (egress description fixed, open egress accepted inline with rationale); dependabot across 6 ecosystems; gitleaks secret scan; runbooks x4 + ADRs 0001-0003; CONTRIBUTING/CODEOWNERS; Makefile mirrors CI linters | make scan-tf exit 0; Lint workflow green; trivy gate proven locally before enforcing |
...
- A check written after the dangerous call is not a control: the image proxy validated `upstream.url` once `fetch` had already followed the redirect, so the SSRF request was made and only its body was withheld. Decide before the side effect (`redirect: "manual"`), not after.
- Rate limiting behind a proxy keys on the wrong thing by default: every public request reaches the pod from the in-cluster ingress, so a per-IP limiter would have been one global bucket. Behind Cloudflare, `CF-Connecting-IP` is the trustworthy key because the edge overwrites it; a plain `X-Forwarded-For` is client-writable.
- Build a gate before you need it: `ci-nodejs.yml` had no smoke test, so a startup crash would only have surfaced after ArgoCD rolled it out. Adding the test first turned an unverifiable dependency change into a checked one.
